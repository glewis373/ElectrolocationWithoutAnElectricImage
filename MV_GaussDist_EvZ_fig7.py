...
#  Ex1 
soln[0] = gamma * -0.0 / np.sqrt(3) # 0.0, #+- 1.0 / np.sqrt(3) # 1.0 / np.sqrt(2)
#  Ey1 
soln[1] = gamma * -0.0 / np.sqrt(3) # 0.0, #+- 1.0 / np.sqrt(3) # 0.0
#  Ex2 
soln[2] = gamma * -0.0 / np.sqrt(3) # 0.0, #+- 1.0 / np.sqrt(3) # 1.0 / np.sqrt(2)
#  Ey2 
soln[3] = gamma * -0.0 / np.sqrt(3) # 0.0, #+- 1.0 / np.sqrt(3) # 0.0
#  Ez1
soln[4] = gamma * 1.0 #/ np.sqrt(3) # 1.0, #+- 1.0 / np.sqrt(3) # 1.0 / np.sqrt(2)
#  Ez2
soln[5] = gamma * 1.0 #/ np.sqrt(3) # 1.0, #+- 1.0 / np.sqrt(3) # 1.0 / np.sqrt(2)
...
#    av2 = x_all[:min_convbin,9+kk].mean()
    ...
    x_all[:,5]=x_all[:,5]/gamma

    mx1 = x_all[:min_convbin,11+kk].max()
    mx2 = x_all[:min_convbin,5+kk].max()
    mn1 = x_all[:min_convbin,11+kk].min()
    mn2 = x_all[:min_convbin,5+kk].min()
    av1 = x_all[:min_convbin,11+kk].mean()
    av2 = x_all[:min_convbin,5+kk].mean()
...
#    delg = 0.625
    ...
    H, xedges, yedges = np.histogram2d(x_all[:min_convbin,11+kk], x_all[:min_convbin,5+kk], bins=(xedges,yedges))
    H = H.T
    ...
    if (kk==0):
        plt.xlabel('$z_o^{(2)}$',fontsize=18)
        plt.ylabel('$E_z^{(2)}$',fontsize=18)
    if (kk==1):
    ...
    filename=f'MVpdf_x_scl_Ex2vsX2_{kk}.pdf'
